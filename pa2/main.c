...
int main(int argc, char *argv[]) {
    ...
    pipesLog = fopen(pipes_log, "a");
    eventsLog = fopen(events_log, "a");
    ...
    printReceiveAllStareted(get_physical_time(), currentId);
    ...
    sendDone();
    receive_any(&processes[currentId], &msg);
    printReceiveAllDone(get_physical_time(), currentId);
...
void receiveTransfer(Message* msg) {
    ...
    if (transferOrder->s_src == currentId) {
        ...
        changeBalance(get_physical_time(), -transferOrder->s_amount);
    ...
    for (local_id i = 1; i <= numberOfPipes; i++) {
        ...
        if (child > 0) {
            ...
            if (child == 0) {
                ...
                setBalance(get_physical_time(), startBalances[i]);
                break;
            } else {
...
void changeBalance(timestamp_t time, balance_t amount) {
    for (timestamp_t t = lastTimeMoment + 1; t < time; t++) {
        setBalance(t, balanceHistory.s_history[lastTimeMoment].s_balance);
    }
    setBalance(time, balanceHistory.s_history[lastTimeMoment].s_balance + amount);
}
